(function($){
	...
	Jigsaw.prototype = {
		...
		_initOrigin : function(){
			...
			var n = jigsaw.options.num
			for(var i = 1;i<n+1;i++){
				var $li = $('<li></li>')			
				$li[0].style.backgroundImage = 'url(img/'+i+'.jpg)'
				$li[0].index = i
				$li.click(function(){
					$(jigsaw.options.jOrigin+' ul'+' li').eq(jigsaw.options.imageIndex-1)[0].className = ''
		...
		_initContent : function(){
			...
			$(jC+' ul'+' li').css('background-image','url(img/'+imgIndex+'.jpg)')
		...
		Renderoriginli : function(){
			$(this.options.jOrigin+' ul'+' li').eq(this.options.imageIndex-1)[0].className = 'active'
		},
	}
